...
Camera::update() {
	...
	if (kickamp > 0.0f) {
		float amp = kickamp;
		D3DMATRIX matrixKick1, matrixKick2;
		if (currentKick == 1)
		{
			uniMatrixRotateX(matrixKick1, (float) sin(uniTimer() / 20.f) * amp);
			uniMatrixRotateY(matrixKick2, (float) cos(uniTimer() / 27.f) * amp);
			uniMatrixRotateZ(matrixKick2, (float) sin(uniTimer() / 23.f) * amp);
		}
		uniMatrixMultiply(matrixKick1, matrixKick2, matrixKick1);
		uniMatrixMultiply(mat, matrixKick1, mat);
	}
